...
@pytest.mark.bank
def test_calc_deposit_interest_rate(mocker, bank_fixt):
    logger.info("Testing successful calculating deposit interest rate (use STUB)")
    ...
    total_stub_1 = 1104.71
    total_stub_2 = 6101.95

    mocker.patch.object(bank_fixt, 'calc_deposit_interest_rate', side_effect=[total_stub_1, total_stub_2])

    result_1 = bank_fixt.calc_deposit_interest_rate('0001')
    result_2 = bank_fixt.calc_deposit_interest_rate('0002')

    assert result_1 == total_stub_1
    assert result_2 == total_stub_2
...
@pytest.mark.close
@pytest.mark.bank
def test_close_deposit(mocker, bank_fixt):
    logger.info("Testing successful closing of a deposit by a bank client (use MOCK)")

    bank_fixt.register_client(name='Yuliya', client_id='0001')
    bank_fixt.open_deposit_account(client_id='0001', start_balance=1000, years=1)

    mock_result = mocker.patch.object(bank_fixt, 'calc_deposit_interest_rate', return_value=1104.71)
    bank_fixt.close_deposit(client_id='0001')
    mock_result.assert_called_once_with('0001')
...
def test_exchange_currency_byn(mocker, conv):
    logger.info("Testing conversion to default target (BYN) (use STUB)")

    mock_exchange_rates = {'BYN': 1.0, 'USD': 2.7293, 'EUR': 3.1712, 'RUB': 3.8411}
    mocker.patch.object(conv, 'exchange_rates', mock_exchange_rates)

    result_usd = conv.exchange_currency(currency='USD', amount=10)
    result_eur = conv.exchange_currency(currency='EUR', amount=5)

    assert result_usd == (27.29, "BYN")
    assert result_eur == (15.86, "BYN")
...
@pytest.mark.regression
@pytest.mark.converter
def test_currency_exchange_specified_rate(mocker, conv):
    logger.info("Testing conversion to specified rate")

    mock_exchange_rates = {'BYN': 1.0, 'USD': 2.7293, 'EUR': 3.1712, 'RUB': 0.0384}
    mocker.patch.object(conv, 'exchange_rates', mock_exchange_rates)
    ...
    assert result_1 == (8.61, "EUR")
    assert result_2 == (5.81, "USD")
    assert result_3 == (412.92, "RUB")
...
@pytest.mark.converter
def test_conversion_currency_zero_amount2(conv):
    logger.info("Testing zero amount (UNSTABLE)")

    today = datetime.datetime.today().weekday()

    if today < 5:
        result = conv.exchange_currency(currency='USD', amount=0)
        assert result == (0, "BYN")
    else:
        result = conv.exchange_currency(currency='USD', amount=0)
        assert result == (27.29, "BYN")
